...
dataset_dir = "C:/Users/alyss/crowd-funding-analysis/datasets"
    ...
    try:
        ...
        if 'category' in df.columns:
            try:
                ...
                df['subcategory_id'] = df['category'].apply(lambda x: x.get('id', None) if isinstance(x, dict) else None)
                df['subcategory_name'] = df['category'].apply(lambda x: x.get('name', None) if isinstance(x, dict) else None)
                df['category_id'] = df['category'].apply(lambda x: x.get('parent_id', None) if isinstance(x, dict) else None)
                df['category_name'] = df['category'].apply(lambda x: x.get('parent_name', None) if isinstance(x, dict) else None)
                df['topics'] = df['category'].apply(lambda x: x.get('slug', None) if isinstance(x, dict) else None)
        ...
        drop_columns = ['creator', 'profile', 'photo', 'urls', 'video', 'category', 'blurb']
        ...
        selected_features = [
            'launched_at', 'id', 'category_id', 'category_name', 'subcategory_id', 'subcategory_name', 'campaign_summary', 'backers_count', 'usd_pledged',
            'converted_pledged_amount', 'staff_pick', 'fx_rate', 'percent_funded', 'goal', 'country', 'currency', 'state', 'name', 'topics'
        ]
